--[[
 Toliss A20N mapping for the Yawman Arrow By Ryan Mikulovsky, CC0 1.0.
 ...
 Repository at https://github.com/rpmik/Lua-Yawman-Control-Toliss-A20N
]]
...
-- autopilot control
		...
		if sp1_pressed and last_button(SIXPACK_1) then
			...
			if dpad_up_pressed then
				meterA20NInteraction(DPAD_PRESSED,dref_APAirSpeedUp, dref_APAirSpeedUp, 1.0, 2.0) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif dpad_down_pressed then
				meterA20NInteraction(DPAD_PRESSED,dref_APAirSpeedDown, dref_APAirSpeedDown,1.0,2.0)
				DPAD_PRESSED = true
			end
		...
		if sp3_pressed and last_button(SIXPACK_3) then
			...
			if dpad_up_pressed then
				meterA20NInteraction(DPAD_PRESSED, dref_APAdjust.AltUp, dref_APAdjust.AltUp, 1.0, 2.0) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif dpad_down_pressed then
				meterA20NInteraction(DPAD_PRESSED, dref_APAdjust.AltDown, dref_APAdjust.AltDown, 1.0, 2.0)
				DPAD_PRESSED = true
			end
		...
		if sp5_pressed and last_button(SIXPACK_5) then
			...
			if dpad_up_pressed then
				meterA20NInteraction(DPAD_PRESSED,dref_APAdjust.HdgUp, dref_APAdjust.HdgUp, 1.0, 3.0) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif dpad_down_pressed then
				meterA20NInteraction(DPAD_PRESSED, dref_APAdjust.HdgDown, dref_APAdjust.HdgDown, 1.0, 3.0)
				DPAD_PRESSED = true
			end
...
-- parking brake			
		if left_bumper_pressed and last_button(LEFT_BUMPER) then
			...
			if wheel_up_pressed or wheel_down_pressed then
				meterA20NInteraction(BUMPERS_PRESSED, dref_FlightControls.brakesMax, dref_brakesMax, 2.0, 20) -- at around two seconds, use larger increment
...
--[[
			...
			if sp4_pressed and not MULTI_SIXPACK_PRESSED then
				if dpad_up_pressed then
					-- EFB but this doesn't quite work. A20N.
					--set_pilots_head(-0.60079902410507,1.5304770469666,-11.694169998169,306.1875,-17.333335876465)
				else
					-- Glareshield A20N +
					set_pilots_head(0.00091499998234212,1.6555010080338,-13.777183532715,359.625,-10.333330154419)
				end
				MULTI_SIXPACK_PRESSED = true
			elseif sp2_pressed and not MULTI_SIXPACK_PRESSED then
				-- Nav, CDU, Transponder, etc A20N +
				set_pilots_head(0.057879999279976,1.9204499721527,-14.025357246399,0.37500002980232,-73.166664123535)
				MULTI_SIXPACK_PRESSED = true
			elseif sp5_pressed and not MULTI_SIXPACK_PRESSED then
				-- FMS A20N +
				set_pilots_head(-0.26577052474022,1.5600252151489,-14.234129905701,15.160492897034,-53.37865447998)
				MULTI_SIXPACK_PRESSED = true
			elseif sp1_pressed and not MULTI_SIXPACK_PRESSED then
				-- Overhead panel A20N +
				set_pilots_head(-0.00012700000661425,1.6650229692459,-13.724100112915,359.625,60.833343505859)
				MULTI_SIXPACK_PRESSED = true
			elseif sp3_pressed and not MULTI_SIXPACK_PRESSED then
				-- A20N tablet/EFB +
				set_pilots_head(-0.74000698328018,1.7241280078888,-14.126784324646,302.625,-26.710870742798)
				MULTI_SIXPACK_PRESSED = true
			elseif sp6_pressed and not MULTI_SIXPACK_PRESSED then
				-- A20N pilot's view of throttles etc +
				set_pilots_head(-0.5029199719429,1.9994879961014,-13.651991844177,29.092767715454,-43.047756195068)
				MULTI_SIXPACK_PRESSED = true
			end
...
-- DPAD_up mode
		if dpad_up_pressed and last_button(DPAD_UP) then
			...
			if wheel_up_pressed then
				meterA20NInteraction(DPAD_PRESSED, dref_FlightControls.flapsDown, dref_FlightControls.flapsDown, 1.0, 10) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif wheel_down_pressed then
				meterA20NInteraction(DPAD_PRESSED, dref_FlightControls.flapsUp, dref_FlightControls.flapsUp, 1.0, 10) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			end
			...
			if dpad_left_pressed then
				...
				set_pilots_head(-0.5,2,-13.64999961853,0,-7.5000004768372)

			elseif dpad_right_pressed then
				-- Copilot's seat A20N
				set_pilots_head(0.44752198457718,2,-13.64999961853,0,-7.5000004768372)
...
function meterA20NInteraction(boolButtonPressed, strCommandName1, strCommandName2, floatSeconds, floatIntervalSpeed)
...
--set_axis_assignment(RUDDER, "yaw", "normal" )
	...
	do_every_frame("multipressTolissA20N_buttons()")
end
